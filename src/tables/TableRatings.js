        ...
        return (
            <div className="custom-tooltip" style={{ padding: '2px', background: '#fcfcfc', border: '1px solid #555555' }}>
                ...
                {payload.map((rating, i) => (
                    <p>
                        {rating.name}:{rating.payload.rating}
                    </p>
                ))}
            </div>
        );
...
const TableRatings = ({ ratings, leaderboardNames }) => {
    ...
    return (
        <div>
            <LineChart width={1230} height={550} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                />
                <YAxis type="number" dataKey="rating" name="rating" domain={['auto', 'auto']} />
                {ratings.map(
                    (rating, i) =>
                        !rating.hidden && (
                            <Line
                                ...
                                type="linear"
                                name={leaderboardNames[rating.leaderboardId]}
                                dataKey="rating"
                                stroke={colors[i]}
                            />
                        )
